...
export default function setupDevServer(app, cb) {
    ...
    let serverBundle; let clientManifest; let ready;
    ...
    clientCompiler.hooks.done.tap('stats', (stats: any) => {
        ...
        clientManifest = JSON.parse(
            readOutputFile(webpackDevMiddleware.context.outputFileSystem, 'vue-ssr-client-manifest.json')
        );
        update();
    });
    ...
    serverCompiler.watch({}, (err, stats: any) => {
        ...
        // 从内存读取文件夹
        serverBundle = JSON.parse(readOutputFile(msf, 'vue-ssr-server-manifest.json'));
