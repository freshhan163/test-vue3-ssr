...
let readyPromise, renderer, clientRenderHtml = '';

// dev模式下，webpack打包构建
function ssrDevWithBundleRender(server) {
    readyPromise = setupDevServer(server, (bundle, options, clientTemplate) => {
...
export default function ssrDev(app) {
    ...
    return async function (context: Context, next: ()=> void) {
        ...
        context.clientRender = () => clientRenderHtml;
